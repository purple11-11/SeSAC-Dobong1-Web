...
exports.postPlayer = async (req, res) => {
  try {
    ...
    console.log("postPlayer err : ", error);
    res.status(500).send("server error");
  }
};
...
exports.patchPlayer = async (req, res) => {
  try {
    ...
    const updatedPlayer = await Player.update(
      ...
      {
        where: {
          player_id: playerId, // api.http 파일에 1로 지정해 놓음
        },
      }
    );
    ...
    console.log("patchPlayer err : ", error);
    res.status(500).send("server error");
  }
};
...
exports.deletePlayer = async (req, res) => {
  try {
    ...
    if (isDeleted) res.send("삭제 성공!");
    else res.send("삭제 실패");
  } catch (error) {
    console.log("deletePlayer err : ", error);
    res.status(500).send("server error");
  }
};
